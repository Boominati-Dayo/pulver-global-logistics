...
function TrackContent() {
  ...
  return (
    <div className={`min-h-screen ${isDarkMode ? 'bg-gray-900' : 'bg-gray-50'}`}>
      <div className="bg-gradient-to-r from-[#1a365d] to-[#2c5282] text-white py-32">
          ...
          <form onSubmit={handleSearch} className="max-w-2xl mx-auto">
            <div className="flex flex-col sm:flex-row gap-3">
              <div className="flex-1 relative">
                <input type="text" value={trackingNumber} onChange={(e) => setTrackingNumber(e.target.value)} placeholder="Enter tracking number (e.g. SXI123456)" className="w-full px-5 py-3 md:py-4 rounded-xl text-lg bg-white text-gray-900 placeholder-gray-500 shadow-lg focus:ring-2 focus:ring-[#ea580c]" />
                <FaSearch className="absolute right-4 top-1/2 -translate-y-1/2 text-gray-400 text-xl hidden sm:block" />
              </div>
              <button type="submit" disabled={isLoading} className="px-6 md:px-8 py-3 md:py-4 bg-gradient-to-r from-[#ea580c] to-[#f97316] text-white font-bold rounded-xl hover:shadow-lg transition-all flex items-center justify-center gap-2">
      ...
      {shipment && (
        <div className="max-w-5xl mx-auto px-4 py-6 space-y-5">
          ...
          <div className="bg-gradient-to-r from-[#1a365d] to-[#2c5282] rounded-2xl p-5 md:p-6 text-white">
            ...
            <div className="grid grid-cols-[1fr_auto_1fr] gap-4 items-center">
              ...
              <div className="flex flex-col items-center gap-1">
                <FaArrowRight className="text-[#ea580c] text-xl" />
                    ...
                    <div key={index} className="relative pl-12 md:pl-16">
                      ...
                      <div className={`p-3 md:p-4 rounded-xl ${isDarkMode ? 'bg-slate-700' : 'bg-gray-50'}`}>
                        ...
                        <div className="flex items-center gap-2 text-sm">
                          <FaMapMarkerAlt className="text-[#ea580c] flex-shrink-0" />
          ...
          {/* Shipment Details */}
          <div className="bg-white dark:bg-gray-800 rounded-2xl p-5 md:p-6 shadow-lg">
            <h2 className="text-lg font-bold text-[#1a365d] dark:text-white mb-4 flex items-center gap-2"><FaInfoCircle className="text-[#ea580c]" /> Shipment Details</h2>
            <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
              {[{ label: 'Carrier', value: shipment.carrier, icon: FaShip }, { label: 'Mode', value: shipment.shipmentMode, icon: FaTruck }, { label: 'Type', value: shipment.shipmentType || 'Standard', icon: FaBox }, { label: 'Payment', value: shipment.paymentMode, icon: FaShippingFast }].map((item, i) => {
                ...
                return <div key={i} className="bg-gray-50 dark:bg-gray-700/50 rounded-xl p-3 md:p-4">
                  <div className="flex items-center gap-2 mb-2"><Icon className="text-[#ea580c] text-sm" /><span className="text-xs text-gray-500">{item.label}</span></div>
                  <p className="font-semibold text-gray-900 dark:text-white text-sm">{item.value}</p>
                </div>;
              })}
            </div>
            <div className="mt-4 p-4 bg-gradient-to-r from-[#ea580c]/10 to-[#f97316]/10 rounded-xl border border-[#ea580c]/20">
              <div className="flex items-center gap-3">
                <div className="w-12 h-12 bg-gradient-to-br from-[#ea580c] to-[#f97316] rounded-full flex items-center justify-center"><FaCalendarAlt className="text-white text-xl" /></div>
                <div><p className="text-sm text-gray-600 dark:text-gray-400">Expected Delivery</p><p className="font-bold text-[#1a365d] dark:text-[#ea580c] text-lg">{shipment.expectedDeliveryDate || 'To be determined'}</p></div>
              </div>
            </div>
          </div>
          ...
          <div className="grid md:grid-cols-2 gap-4">
            <div className="bg-white dark:bg-gray-800 rounded-2xl p-5 md:p-6 shadow-lg">
              <h2 className="text-lg font-bold text-[#1a365d] dark:text-white mb-4 flex items-center gap-2"><span className="w-8 h-8 bg-[#1a365d] text-white rounded-full flex items-center justify-center text-sm font-bold">S</span>Shipper</h2>
              <div className="space-y-3">
                <div className="flex items-start gap-3"><FaUser className="text-[#ea580c] mt-0.5" /><div><p className="font-medium text-gray-900 dark:text-white">{shipment.shipperName}</p><p className="text-sm text-gray-500 break-words">{shipment.shipperAddress}</p></div></div>
                <div className="flex items-center gap-3"><FaPhone className="text-[#ea580c]" /><span className="text-sm text-gray-900 dark:text-white break-all">{shipment.shipperPhone}</span></div>
                <div className="flex items-center gap-3"><FaEnvelope className="text-[#ea580c]" /><span className="text-sm text-gray-900 dark:text-white break-all">{shipment.shipperEmail}</span></div>
              </div>
            </div>
            <div className="bg-white dark:bg-gray-800 rounded-2xl p-5 md:p-6 shadow-lg">
              <h2 className="text-lg font-bold text-[#1a365d] dark:text-white mb-4 flex items-center gap-2"><span className="w-8 h-8 bg-gradient-to-br from-[#ea580c] to-[#f97316] text-white rounded-full flex items-center justify-center text-sm font-bold">R</span>Receiver</h2>
              <div className="space-y-3">
                <div className="flex items-start gap-3"><FaUser className="text-[#ea580c] mt-0.5" /><div><p className="font-medium text-gray-900 dark:text-white">{shipment.receiverName}</p><p className="text-sm text-gray-500 break-words">{shipment.receiverAddress}</p></div></div>
                <div className="flex items-center gap-3"><FaPhone className="text-[#ea580c]" /><span className="text-sm text-gray-900 dark:text-white break-all">{shipment.receiverPhone}</span></div>
                <div className="flex items-center gap-3"><FaEnvelope className="text-[#ea580c]" /><span className="text-sm text-gray-900 dark:text-white break-all">{shipment.receiverEmail}</span></div>
              </div>
            </div>
          </div>

          {/* Help */}
          <div className="bg-gradient-to-r from-[#ea580c] to-[#f97316] rounded-2xl p-5 md:p-6 shadow-lg">
            <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
              <div className="flex items-center gap-4">
                <div className="w-14 h-14 bg-[#1a365d] rounded-full flex items-center justify-center"><FaHeadset className="text-[#ea580c] text-2xl" /></div>
              ...
              <a href="/contact" className="px-6 py-3 bg-[#1a365d] text-white font-bold rounded-xl hover:bg-[#0f2340] transition-colors text-center">Contact Support</a>
            </div>
          </div>
        </div>
      )}
      ...
      {!shipment && !isLoading && (
        <div className="max-w-6xl mx-auto px-4 py-12">
          <div className="text-center mb-10">
            <h2 className="text-2xl font-bold text-[#1a365d] dark:text-white mb-3">Why Track With SXI?</h2>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-5">
            {[{ icon: FaMapMarkerAlt, title: 'Real-time Tracking', desc: 'GPS accuracy monitoring' }, { icon: FaShieldAlt, title: 'Secure Handling', desc: 'Fully insured shipments' }, { icon: FaBox, title: 'Detailed Updates', desc: 'Complete shipment history' }, { icon: FaHeadset, title: '24/7 Support', desc: 'Expert assistance anytime' }, { icon: FaBolt, title: 'Fast Delivery', desc: 'Express options available' }, { icon: FaGlobe, title: 'Global Coverage', desc: 'Track worldwide' }].map((feature, index) => {
              ...
              return <div key={index} className="text-center p-5 bg-white dark:bg-gray-800 rounded-2xl shadow-sm"><div className="w-14 h-14 bg-[#ea580c]/10 rounded-full flex items-center justify-center mx-auto mb-4"><Icon className="text-2xl text-[#ea580c]" /></div><h3 className="text-lg font-bold text-[#1a365d] dark:text-white mb-2">{feature.title}</h3><p className="text-gray-600 dark:text-gray-400 text-sm">{feature.desc}</p></div>;
            })}
          </div>
        </div>
      )}
...
export default function Track() {
  return (
    <Suspense fallback={<div className="min-h-screen flex items-center justify-center"><FaSpinner className="animate-spin text-3xl text-[#ea580c]" /></div>}>
      <TrackContent />
    </Suspense>
  );
}